...
from decimal import Decimal
from config import Config
from database.db_objects import DETALLE_VENTAS, PRODUCTOS, VENTAS


# =============================
# TOP PRODUCTOS VENDIDOS
# =============================
def get_top_productos(cursor):
    ...
    query = f"""
        SELECT
            p.nombre,
            SUM(dv.cantidad) AS cantidad,
            SUM(dv.cantidad * dv.precio) AS ventas
    ...
    for nombre, cantidad, ventas in cursor.fetchall():
...
def get_dashboard():
    ...
    try:
        ...
        cantidad_ventas, ventas_dia = cursor.fetchone()

        return {
            "ventas_dia": float(Decimal(ventas_dia or 0)),
            "cantidad_ventas": int(cantidad_ventas or 0),
            "top_productos": get_top_productos(cursor)[:5]
        }
